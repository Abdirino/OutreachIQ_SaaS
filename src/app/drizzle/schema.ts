...
export const ProductTable = pgTable(
  "products",
  {
    ...
    clerkUserId: text("clerk_use_ud").notNull(),
...
export const ProductCustomizationTable = pgTable("product_customizations", {
  ...
  locationMessage: text("location_message")
    .notNull()
    .default(
      "Hey! it looks like you're from <b>country</b>. we support parity purchasing power, so if you need it, use code <b>{coupon}</b> to get <b>discount</b> off"
    ),
  backgroundColor: text("background_color")
    .notNull()
    .default("hsl(193, 82%, 31%)"),
  textColor: text("text_color").notNull().default("hs1(0,0%,100%)"),
